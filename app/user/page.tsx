...
export default function CollapsibleTable() {
  ...
  useEffect(() => {
    fetchUsers().then(({ users, deletedUsers }) => {
      setData(users);
      setArchivedData(deletedUsers)
    });
  }, []);
    ...
    try {
      ...
      if (response.data.status === 'success') {
        new Toast({
          position: "bottom-right",
          onClose: () => { window.location.href = "/user"; },
          toastMsg: "Successfully restored user!",
          autoCloseTime: 1000,
          canClose: true,
          showProgress: true,
          pauseOnHover: true,
      ...
      new Toast({
        position: "top-right",
        toastMsg: "Error restoring transaction",
        autoCloseTime: 1000,
        canClose: true,
        showProgress: true,
        pauseOnHover: true,
          ...
          <div className="mb-4 flex justify-end col-span-2 relative">
            ...
            <div className="flex">
              ...
              <div className="relative ml-1">
                ...
                <div
                  className={`absolute right-0 mt-1 z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 transition-all duration-300 ease-in-out transform ${ellipsisDropdownOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 hidden"
                    }`}
                >
            ...
            <div
              id="dropdownContent"
              className={`overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-gray-900 bg-opacity-50 ${restoreView ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 hidden"
                }`}
            >
              <div className="relative p-4 md:p-6 w-full max-w-2xl max-h-full">
                <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                  ...
                  <div className="p-4 md:p-5">
                    ...
                    {archivedData?.map((user, index) => (
                      <div key={index} className="mb-4 bg-gray-50 dark:bg-gray-800 rounded-md">
                        <div className="flex items-center justify-between p-4">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-600 dark:text-gray-300">{user?.role.charAt(0).toUpperCase() + user?.role.slice(1)}</p>
                            </div>
                          </div>
